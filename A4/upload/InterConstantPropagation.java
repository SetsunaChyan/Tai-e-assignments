...
public class InterConstantPropagation extends
    ...
    protected CPFact transferCallEdge(CallEdge<Stmt> edge, CPFact callSiteOut) {
        ...
        for (int i = 0; i < params.size(); i++)
            ret.update(params.get(i), callSiteOut.get(invoke.getInvokeExp().getArg(i)));
        return ret;
    }
    ...
    protected CPFact transferReturnEdge(ReturnEdge<Stmt> edge, CPFact returnOut) {
        CPFact ret = new CPFact();
        Stmt source = edge.getCallSite();
        if (source instanceof DefinitionStmt<?, ?> definitionStmt) {
            LValue lv = definitionStmt.getLValue();
            if (lv instanceof Var var)
                for (Var retVars : edge.getReturnVars())
                    ret.update(var, cp.meetValue(ret.get(var), returnOut.get(retVars)));
        }
        return ret;
    }
}

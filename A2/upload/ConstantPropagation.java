    ...
    public static Value evaluate(Exp exp, CPFact in) {
        ...
        if (exp instanceof BinaryExp binaryExp) {
            ...
            if (binaryExp instanceof ArithmeticExp aExp) {
                switch (aExp.getOperator()) {
                    case MUL:
                        if (v2.isConstant() && v2.getConstant() == 0) return Value.makeConstant(0);
                    case DIV:
                    case REM:
                        if (v1.isConstant() && v1.getConstant() == 0) return Value.makeConstant(0);
                }
            }

            if (binaryExp instanceof ShiftExp sExp) {
            ...
            if (binaryExp instanceof BitwiseExp bExp) {
                if (bExp.getOperator() == BitwiseExp.Op.AND) {
                    if (v1.isConstant() && v1.getConstant() == 0) return Value.makeConstant(0);
                    if (v2.isConstant() && v2.getConstant() == 0) return Value.makeConstant(0);
                }
            }*/
